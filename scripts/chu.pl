...
GetOptions(
           "X"       => \$pairs,      # digraph or contact frequency
...
my ($XYZ, $abc) = (
                   (
                    join q{}, sort { $Xlate{$a} cmp $Xlate{$b} }
                      keys %Xlate
                   ),
                   (
                    join q{},
...
=head2  -W  wordsLike(), shape dictionary %WordsByShape, ordercannon()
...
Uri and Bob Rogers optimized ordercannon() with C<s///ge> instead of C<s///g for split>.
...
sub wordsLike
{
    ...
    if ($partial =~ s/s$/s?/)
    {
        ...
        push @W, $_ for grep /^$partial$/i, @{$WordsByShape{$cannon}};
...
if (0 && $cheat)
{
    # simple, but -X, -W don't see it ... so moved earlier.
